...
const AgentMapArrivalScreen1 = ({navigation}) => {
  const mapRef = useRef();
  ...
  const getLiveLocation = async () => {
    // console.log('livelocatio');
    try {
      const locPermissionGranted = await locationPermission();
      if (locPermissionGranted === 'granted') {
        const {latitude, longitude, heading} = await getCurrentLocation();
        console.log('latintidfdf', latitude);
...
export default AgentMapArrivalScreen1;
