  ...
  const finalizeApprovedCall = async () => {
    ...
    const renderParticipantPips = () => (
      <View pointerEvents="box-none" style={styles.participantPipStack}>
        {/* A notary session has one client and one agent. Agora can briefly
          report a second UID while a participant reconnects; only render the
          active counterpart alongside the local participant. */}
        {remoteUids.slice(0, 1).map((remoteUid, index) => (
